...
from OCR.document_OCR.dan_plus.trainer_danp import Manager
from OCR.document_OCR.dan_plus.models_danp import GlobalHTADecoder
...
def train_and_test(rank, params, test, load_epoch):
    ...
    for dataset_name in params["dataset_params"]["datasets"].keys():
        for set_name in ['test', "valid", 'train']:
            model.predict("{}-{}".format(dataset_name, set_name), [(dataset_name, set_name), ], metrics, ema)
...
if __name__ == "__main__":
    ...
    if 'skip' in params['model_params']:
        params['dataset_params']['config']['skip'] = params['model_params']['skip']
    ...
    params['training_params']['max_training_time'] = 3600 * 24 * 3.9
