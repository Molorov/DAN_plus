        ...
        for b in range(len(y_len)):
            for i in range(1, y_len[b]):
                if np.random.rand() < error_rate and y[b][i] != self.dataset.tokens["pad"]:
                    y_error[b][i] = np.random.randint(0, len(self.dataset.class_set)+2)
    ...
    def evaluate_batch(self, batch_data, metric_names, **kwargs):
        ...
        with autocast(enabled=self.params["training_params"]["use_amp"]):
            ...
            for i in range(0, max_chars):
                output, pred, hidden_predict, cache, weights = self.models["decoder"](features, enhanced_features, predicted_tokens, reduced_size, predicted_tokens_len, features_size, start=0, hidden_predict=hidden_predict, cache=cache, num_pred=1, keep_all_weights=keep_all_weights)
